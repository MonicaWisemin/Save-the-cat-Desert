...
while run:
    ...
    if start_game == False:
        ...
        screen.blit(resized_stbg,(0,0))
        screen.blit(resized_stitle, (5, 5))
        screen.blit(re_stmenu, (720, 300))
        ...
        if startButton.draw(screen):
            ...
            print('Game Quit!')
            run = False
        elif collectionButton.draw(screen):
            print('Working on it!')

    else:
        game_totalT = 5*60*1000
        game_totalT -= 1000
        ...
        if not cati.isalive:
            ...
            #if ghost rect reaches cat rect:
                #cat.update_action(1)
            #else:
                #cat.update_action(0)
            #check if player is alive, if is, then update the actions
...
#close the game when hit the x button on the right-up side of the window
    for event in pygame.event.get() :
        ...
        '''
            #ghost appears
        if event.type == pygame.USEREVENT and not ghosti.moving:
            ghosti.random_show()
        if not ghosti.moving:
